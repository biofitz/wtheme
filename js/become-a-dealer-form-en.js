...
function dealerFileCheck(el){
	...
	if(el.val() == ""){
		...
		el.next().html("Attach the archive <img src='/wp-content/themes/winner-ua/img/general/file_arrow.png'' alt=''/>");
	}
	else{
...
function dealerSendForm(form, url){
	...
	$.ajax({
		...
		error: function(jqXHR, textStatus, errorThrown){
			...
			alert("It is difficult for the server to process your file. You may be trying to upload a too large file. Try again, please." );
...
function closeDealerPopup(){
	function close(){
		...
		setTimeout(function(){
			...
			$("#dealer_file").val("");
			$("#field_dealer_button").html("Attach the archive <img src='/wp-content/themes/winner-ua/img/general/file_arrow.png'' alt=''/>");
